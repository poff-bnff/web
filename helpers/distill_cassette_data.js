...
const DISTILLED_PROGRAMMES = STRAPIDATA['Programme'].map(s_programme => {
    ...
    d_programme.search = festivals.map(festival => {
        try {
            ...
            timer.log('debug', {festival_edition, error})
            throw error
        }
    })
    return d_programme
...
function tokenize(arr, id, lang) {
    ...
    try {
        ...
        timer.log(__filename, {INFO: 'no freetext search for', id, lang})
        return ''
    }
}
...
function distill_strapi_cassette(s_cassette, s_films, s_screenings, lang) {
    ...
    function add_search_to_cassette(cassette, lang) {
        cassette.search = {
            ...
            languages: cassette.films.map(film => film._languageCodes).flat(),
            countries: cassette.films.map(film => film._countryCodes).flat(),
